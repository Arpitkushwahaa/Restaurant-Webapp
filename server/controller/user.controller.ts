...
export const signup = async (req: Request, res: Response) => {
    try {
        ...
        try {
            ...
            // Check for duplicate key error (MongoDB specific)
...
export const checkAuth = async (req: Request & { id?: string }, res: Response) => {
    try {
...
export const updateProfile = async (req: Request & { id?: string }, res: Response) => {
    try {
        ...
        // Create updatedData object with basic fields
        const updatedData: any = { fullname, email, address, city, country };
        
        // Only upload image to cloudinary if profilePicture is provided and is a valid data URL
        if (profilePicture && profilePicture.startsWith('data:image')) {
            try {
                const cloudResponse = await cloudinary.uploader.upload(profilePicture);
                // Add the image URL to updatedData
        ...
        // For development, if userId doesn't exist or database is not connected, return a mock response
        ...
        // If we're in production, update the user in the database
